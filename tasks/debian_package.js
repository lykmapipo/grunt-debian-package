...
module.exports = function(grunt) {
    ...
    var _copy = fileSystem._copy(grunt);
    var _cleanUp = fileSystem._cleanUp(grunt);
    var _findAndReplace = replace._findAndReplace(grunt);
    var _transformAndReplace = replace._transformAndReplace(grunt);
    var preparePackageContents = function(makefile, files, follow_soft_links, quiet) {
    ...
    grunt.registerMultiTask('debian_package', 'Create debian package from grunt build', function() {
        ...
        var now = dateFormat(new Date(), 'ddd, d mmm yyyy h:MM:ss +0000');
        var temp_directory = options.working_directory + options.packaging_directory_name;
        var controlDirectory = temp_directory + '/debian';
        var changelog = controlDirectory + '/changelog';
        var control = controlDirectory + '/control';
        var links = controlDirectory + '/links';
        var dirs = controlDirectory + '/dirs';
        var makefile = temp_directory + '/Makefile';
        var dependencies = '';
        var pre_dependencies = '';
        ...
        _findAndReplace([control], '\\$\\{home_page\\}', options.home_page);
        _findAndReplace([control], '\\$\\{bugs_page\\}', options.bugs_page);
